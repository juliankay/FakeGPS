...
--*/
HRESULT CSensorDDI::UpdateSensorPropertyValues(
    ...
    _In_ BOOL fSettableOnly)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
            CGeolocationSensor* pGeolocation = nullptr;

            switch (sensType)
            {
                case Geolocation:
                    pGeolocation = static_cast<CGeolocationSensor*>(pSensor);
...
CSensorDDI::RequestDeviceInfo
    ...
    Synchronously request the report descriptor, which contains the sensor type.
...
CSensorDDI::OnGetSupportedProperties
    ...
    This method is called by Sensor Class Extension to get the list of supported properties for a particular Sensor.
...
CSensorDDI::OnGetSupportedDataFields
    ...
    This method is called by Sensor Class Extension to get the list of supported data fields for a particular Sensor.
...
CSensorDDI::OnGetSupportedEvents
    ...
    This method is called by Sensor Class Extension to get the list of supported events for a particular Sensor.
...
CSensorDDI::OnGetProperties
    ...
    This method is called by Sensor Class Extension to get Sensor property values for a particular Sensor.
...
CSensorDDI::OnGetRadioState
    ...
    This method is called for Radio Management IO and gets the current radio state.
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (nullptr != spOutputMemory)
        {
            ...
            if (sizeof(DEVICE_RADIO_STATE) == cbBufferOut && nullptr != pOutBuffer)
            {
                ...
                if (SUCCEEDED(hr) && nullptr != pSensor)
                {
                    CGeolocationSensor* pGeolocation = (CGeolocationSensor*) pSensor;
...
CSensorDDI::OnGetDataFields
    ...
    This method is called by Sensor Class Extension to get Sensor data fields for a particular Sensor.
...
The parameters sent to us are:
    ...
    pValues - an IPortableDeviceValues which will contain the property values retrieved from the object

The driver should:
    Read the specified properties for the specified object and populate pValues with the results.
...
CSensorDDI::OnSetProperties
    ...
    This method is called by Sensor Class Extension to set Sensor properties for a particular Sensor.
...
Arguments:
    pRequest        - The IO request
    fPreviousState  - If the previous radio state should be set instead of the current state.
...
CSensorDDI::OnClientConnect
    ...
    This method is called by Sensor Class Extension when a client app connects to a Sensor
...
CSensorDDI::OnClientDisconnect
 ...
 This method is called by Sensor Class Extension when a client app disconnects from a Sensor
    ...
    _In_ LPWSTR pwszObjectID)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr) && (nullptr != pSensor))
        {
            ...
            {
                hr = E_UNEXPECTED;
                Trace(TRACE_LEVEL_ERROR, "Attempt to remove non-existent client, hr = %!HRESULT!", hr);
            }
        }
        else
        {
...
CSensorDDI::OnClientSubscribeToEvents
    ...
    This method is called by Sensor Class Extension when a client subscribes to events by calling SetEventSink
...
CSensorDDI::OnClientUnsubscribeFromEvents
    ...
    This method is called by Sensor Class Extension when a client unsubscribes from events by calling SetEventSink(NULL)
...
CSensorDDI::OnGetPropertyValues
    ...
    _Out_ IPortableDeviceValues** ppValues)
{
    ...
    if (hr == S_OK && ppValues != NULL)
    {
        ...
        CGeolocationSensor*       pGeolocation = NULL;
            ...
            if (hr == S_OK)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if ((NULL != pSensor) && (SUCCEEDED(hr)))
                    {
                        ...
                        {
                            case Geolocation:
                                pGeolocation = (CGeolocationSensor*) pSensor;
...
CSensorDDI::FindClientFromAppID

    This method is called to determine the client index and the client-desired settings for settable properties.
...
--*/
HRESULT CSensorDDI::FindClientFromAppID(
...
--*/
HRESULT CSensorDDI::CheckForClient(
    _In_ CSensor* pSensor,
    _In_ IWDFFile* appID,
...
--*/
HRESULT CSensorDDI::RemoveClient(
    _In_ CSensor* pSensor,
    _In_ IWDFFile* appID)
{
    HRESULT hr = S_OK;
...
--*/
HRESULT CSensorDDI::CheckForSubscriber(
    _In_ CSensor* pSensor,
    _In_ IWDFFile* appID,
...
--*/
HRESULT CSensorDDI::RemoveSubscriber(
    _In_ CSensor* pSensor,
    _In_ IWDFFile* appID)
{
    HRESULT hr = S_OK;
...
FindSensorTypeFromObjectID

    This method is called to determine the sensor type and index from the object ID.
...
--*/
HRESULT CSensorDDI::FindSensorTypeFromObjectID(
    _In_ LPWSTR pwszObjectID,
...
CSensorDDI::IsSettablePropertyKey

    This method is called to determine if the given PROPERTYKEY is a settable property of the SENSOR object.
...
--*/
BOOL CSensorDDI::IsSettablePropertyKey(
    _In_ DWORD sIndex,
...
CSensorDDI::FindPropertyKey

    This method is called to determine if the given PROPERTYKEY can be found in the given collection
...
--*/
BOOL CSensorDDI::FindPropertyKey(
    _In_ PROPERTYKEY Key,
    _In_ IPortableDeviceKeyCollection *pKeys)
{
    HRESULT hr = S_OK;
...
CSensorDDI::AddCommonPropertyKeys
...
CSensorDDI::AddSensorPropertyKeys
...
CSensorDDI::AddSensorDataFieldKeys
...
CSensorDDI::AddSupportedPropertyKeys
...
CSensorDDI::AddSupportedDataFieldKeys
...
CSensorDDI::SelectClientReportInterval
...
CSensorDDI::SelectClientLocationDesiredAccuracy
...
CSensorDDI::SelectClientChangeSensitivity
