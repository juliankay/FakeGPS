...
CSensorManager::Initialize
    ...
    Merely store the device pointer.
    The rest of the init will be done in Start().
    This is because init depends on communication with the device.

--*/
HRESULT CSensorManager::Initialize(_In_ IWDFDevice* pWdfDevice, _In_ CFakeGPS* pDevice)
{
                        ...
                        else
                        {
                            switch (m_AvailableSensorsTypes[idx])
                            {
                                ...
                                case Geolocation:
                                    ...
                                    try
                                    {
                                        pGeolocation = new CGeolocationSensor();
                                    }
                                    catch (...)
                                    {
                                        hr = E_UNEXPECTED;
...
#pragma warning(suppress: 6001) // uninitialized memory
    ...
    // Note: CSensorManager::_SensorEventThreadProc() will change sensor state to SENSOR_STATE_READY whenever a data field is ready.
    if (SUCCEEDED(hr))
    {
        for (DWORD i = 0; i < m_pSensorList.GetCount(); i++)
        {
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            m_hSensorManagerEventingThread = ::CreateThread(
                NULL,                                       // Cannot be inherited by child process
...
--*/
HRESULT CSensorManager::SetState(
    _In_ LPVOID pvData,
    _In_ SensorState newState,
    _Out_ bool* pfStateChanged)
{
...
--*/
HRESULT CSensorManager::PostDataEvent(
...
--*/
HRESULT CSensorManager::PostStateChange(
    _In_ LPWSTR objectId,
    _In_ SensorState newState)
{
...
CSensorManager::ProcessIoControlRadioManagement
    ...
    This method is called to process a Device IO Control for Radio Management.
