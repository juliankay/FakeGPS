...
Description:
    ...
    Implements the CGeolocationSensor container class
...
#include "GeolocationSensor.tmh"

#if USE_GEOLOCATION_SPECIFIC_UNIQUE_ID
// {A3921B4F-BEEF-BEEF-94DB-47F59CBDE2C9}
static const GUID SENSOR_ID_FAKEGPS =
{ 0xa3921b4f, 0xbeef, 0xbeef, { 0x94, 0xdb, 0x47, 0xf5, 0x9c, 0xbd, 0xe2, 0xc9 } };
#endif
...
CGeolocationSensor::CGeolocationSensor
...
CGeolocationSensor::Initialize

    Initializes the PROPERTYKEY/PROPVARIANT values for the Supported Properties & Supported Data

--*/
HRESULT CGeolocationSensor::Initialize(
...
CGeolocationSensor::InitializeGeolocation

    Initializes the Geolocation PropertyKeys and DataFieldKeys

--*/
HRESULT CGeolocationSensor::InitializeGeolocation(_In_ IWDFDevice* pWdfDevice)
{
...
CGeolocationSensor::AddGeolocationPropertyKeys

    Copies the PROPERTYKEYS for Geolocation Supported Properties and sets the Values to VT_EMPTY

--*/
HRESULT CGeolocationSensor::AddGeolocationPropertyKeys()
{
    HRESULT hr = S_OK;
...
CGeolocationSensor::AddGeolocationSettablePropertyKeys

    Copies the PROPERTYKEYS for Geolocation Supported Properties and sets the Values to VT_EMPTY

--*/
HRESULT CGeolocationSensor::AddGeolocationSettablePropertyKeys()
{
    HRESULT hr = S_OK;
...
CGeolocationSensor::AddGeolocationDataFieldKeys

    Copies the PROPERTYKEYS for Geolocation Supported DataFields and sets the Values to VT_EMPTY

--*/
HRESULT CGeolocationSensor::AddGeolocationDataFieldKeys()
{
    HRESULT hr = S_OK;
...
CGeolocationSensor::SetGeolocationDefaultValues

    Fills in default values for most Geolocation Properties and Data Fields.

--*/
HRESULT CGeolocationSensor::SetGeolocationDefaultValues(_In_ IWDFDevice* pWdfDevice)
{
    HRESULT hr = S_OK;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        // Will use as a default, if not set
        LPWSTR lat = L"1.31337";

        hr = spPropStore->GetNamedValue(PROP_STORE_KEY_LATITUDE, &var);
        if (SUCCEEDED(hr))
        {
            lat = var.pwszVal;
        }
        ...
        {
            PropVariantInit(&var);
            var.vt = VT_LPWSTR; //pwszVal
            var.pwszVal = lat;
            hr = spPropStore->SetNamedValue(PROP_STORE_KEY_LATITUDE, &var);
            PropVariantClear(&var);
        }

        m_Latitude = _tstof(lat);
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        // Will use as a default, if not set
        LPWSTR lon = L"1.31337";

        hr = spPropStore->GetNamedValue(PROP_STORE_KEY_LONGITUDE, &var);
        if (SUCCEEDED(hr))
        {
            lon = var.pwszVal;
        }
        else
        {
            ...
            var.vt = VT_LPWSTR; //pwszVal
            var.pwszVal = lon;
            hr = spPropStore->SetNamedValue(PROP_STORE_KEY_LONGITUDE, &var);
            PropVariantClear(&var);
        }

        m_Longitude = _tstof(lon);
    }
...
CGeolocationSensor::GetPropertyValuesForGeolocationObject
...
CGeolocationSensor::UpdateGeolocationPropertyValues

    This method updates the writable properties of the sensor.

--*/
HRESULT CGeolocationSensor::UpdateGeolocationPropertyValues()
{
...
CGeolocationSensor::UpdateGeolocationDataValues

    This method sends a poll data request to the device.

--*/
HRESULT CGeolocationSensor::UpdateGeolocationDataFieldValues()
{
    HRESULT hr = S_OK;
    ...
    if (m_pSensorManager->m_fDeviceActive)
    {
        ...
        if (fContinue)
        {
            ...
            // move out of loop?
            CComPtr<IWDFNamedPropertyStore> spPropStore;
            hr = m_pWdfDevice->RetrieveDevicePropertyStore(NULL, WdfPropertyStoreCreateIfMissing, &spPropStore, NULL);
            ...
            DOUBLE dblNewLatitude               = m_Latitude;
            DOUBLE dblNewLongitude              = m_Longitude;

            // NOTE: these guys are currently static values
            const DOUBLE dblNewErrorRadius      = 4.321;
            const DOUBLE dblNewAltitude         = 0;
            const DOUBLE dblNewAltitudeError    = 9.876;
            const DOUBLE dblNewSpeed            = 0;
            const DOUBLE dblNewHeading          = 0;
...
HRESULT CGeolocationSensor::SetUniqueID(_In_ IWDFDevice* pWdfDevice)
{
    HRESULT hr = S_OK;

    UNREFERENCED_PARAMETER(pWdfDevice);

    hr = m_spSensorPropertyValues->SetGuidValue(SENSOR_PROPERTY_PERSISTENT_UNIQUE_ID, SENSOR_ID_FAKEGPS);

    return hr;
}
#endif

HRESULT CGeolocationSensor::SetState(_In_ SensorState newState, _Out_ bool* pfStateChanged)
{
